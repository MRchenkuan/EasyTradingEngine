...
风控层      PositionController（8 级复合风险） / AccountRiskMonitor（回撤监控） / Liquidator（一键清仓）
   ↓
展示层      VisualEngine（Chart.js 4K 渲染） + 监控面板（Express + WebSocket :8080）
```
...
- **新技术指标**：在 `src/indicators/` 下新建，纯函数实现，在阈值计算或绘图模块中接入
- **新图表**：继承 `src/TradeEngine/painters/AbstractPainter.js`，在 VisualEngine 注册
